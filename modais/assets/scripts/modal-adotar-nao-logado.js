...
async function showModalEntrarEmContatoo(data) {
  ...
  async function fetchUser(userId) {
    try {
      const response = await fetch(`https://jsonserver-usuarios-helpet--imcathalat1.repl.co/users/${userId}`);
  ...
  async function renderUserInformationForContact(){
    ...
    showModalEntrarEmContatoo(data);
  }
